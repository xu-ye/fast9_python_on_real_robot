...
def real_angles_to_sim2(real_angles):
    # theta [6,3]
    ...
    for i in range(6):
        if i<3:
            theta[i,0]= (real_angles_2[3+i*6+0]-180)/180.0*math.pi
            theta[i,1]= (-real_angles_2[3+i*6+1]+180)/180.0*math.pi
            theta[i,2]= (real_angles_2[3+i*6+2]-62.69)/180.0*math.pi
            
            
            
        else:
            theta[i,0]= (real_angles_2[0+(i-3)*6+0]-180)/180.0*math.pi
            theta[i,1]= (real_angles_2[0+(i-3)*6+1]-180)/180.0*math.pi
            theta[i,2]= (-real_angles_2[0+(i-3)*6+2]+62.69)/180.0*math.pi
            
           
        
    return theta

def sim_angles_to_real2(theta):
...
def actuator_model_error(P_gain,D_gain,max_speed):
    ...
    with open('/home/xu-ye/Downloads/record-main/servo_all_fre_16.csv') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                frictionForce=0.9
                p.setJointMotorControl2(robot, 0, p.VELOCITY_CONTROL, targetVelocity=0, force=frictionForce)
                p.setJointMotorControl2(bodyUniqueId=robot,jointIndex=0,controlMode=p.POSITION_CONTROL,targetPosition=goal_position,
                positionGain=P_gain,
                velocityGain=D_gain,
...
def actuator_control_pd_force(P_gain,D_gain,friction):
    p.connect(p.DIRECT )  # 连接到仿真服务器
    #p.connect(p.GUI )  # 连接到仿真服务器
    p.setGravity(0, 0, -9.8)  # 设置重力值
    p.setAdditionalSearchPath(pd.getDataPath())  # 设置pybullet_data的文件路径
    ...
    with open('/home/xu-ye/Downloads/record-main/servo_all_fre_10_2.csv') as csv_file:
        ...
        for row in csv_reader:
            ...
            if line_count == 0:
                ...
                tau=explicitPD.computePD(robot,[0],goal_position,0,P_gain,D_gain,Force,dt,first_one)
                first_one=0
                real_tau=1.82*2.68*eval(row[3])/1000

                print("cal tau",tau,"    real tau",real_tau)
                p.setJointMotorControl2(robot, 0, p.VELOCITY_CONTROL, targetVelocity=0, force=friction)
                # 直接应用real tau 完全没有按照既定轨迹运行
                p.setJointMotorControl2(bodyUniqueId=robot,jointIndex=0,controlMode=p.TORQUE_CONTROL,
                force = tau)
                p.stepSimulation()
                #sim_position=get_obseration(robot)
                #sim_position_tick=sim_position/math.pi*2048+2048
...
def actuator_control_pd__multi(P_gain,D_gain,max_speed):
    #p.connect(p.DIRECT )  # 连接到仿真服务器
    p.connect(p.GUI )  # 连接到仿真服务器
    p.setGravity(0, 0, -9.8)  # 设置重力值
    p.setAdditionalSearchPath(pd.getDataPath())  # 设置pybullet_data的文件路径
...
def actuator_control_pd_force_multi(P_gain,D_gain,friction):
    #p.connect(p.DIRECT )  # 连接到仿真服务器
    p.connect(p.GUI )  # 连接到仿真服务器
    p.setGravity(0, 0, -9.8)  # 设置重力值
    p.setAdditionalSearchPath(pd.getDataPath())  # 设置pybullet_data的文件路径
    ...
    with open('/home/xu-ye/Downloads/dynamixel_workbench_master-main(1)/dynamixel_workbench_master-main/dynamixel_workbench_toolbox/test/build/servo_all_fre_cpg_0.2.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        skip_flag=0
        for row in csv_reader:
            ...
            if line_count == 0:
                ...
                tau=explicitPD.computePD(robot,range(18),goal_pos_sim,[0]*18,[P_gain]*18,[D_gain]*18,[Force]*18,dt,first_one)
...
def actuator_control_pd_force_multi_stable(P_gain,D_gain,friction):
    p.connect(p.DIRECT )  # 连接到仿真服务器
    #p.connect(p.GUI )  # 连接到仿真服务器
    p.setGravity(0, 0, -9.8)  # 设置重力值
    p.setAdditionalSearchPath(pd.getDataPath())  # 设置pybullet_data的文件路径
    ...
    stablePD=PDControllerStable(p)
    explicitPD_mul=PDControllerExplicitMultiDof(p)
    stable_PD_mul=PDControllerStableMultiDof(p)
    step=0
    all_count=0
    print_flag=0

    with open('/home/xu-ye/Downloads/dynamixel_workbench_master-main(1)/dynamixel_workbench_master-main/dynamixel_workbench_toolbox/test/build/servo_all_fre_cpg_0_5.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        skip_flag=0
        for row in csv_reader:
            ...
            if line_count == 0:
                ...
                if print_flag:
                    ...
                    print("real position",current_pos)
                    print("position now",sim_position_tick,)
                error_sum+=np.absolute(sim_real_error)
                all_count+=1
                center_pos=np.array([0,0,0,0,0,0,0])
                center_vel=[0,0.1,0,0,0,0,0]
                
                goal_pos_25=np.concatenate([center_pos,goal_pos_sim])
                
                goal_vel_25=np.array(center_vel+[0]*18)
                #tau=explicitPD_mul.computePD(robot,range(18),goal_pos_sim,[0]*18,P_gain,D_gain,[Force]*18,dt)
                #tau=explicitPD.computePD(robot,range(18),goal_pos_sim,[0]*18,[P_gain]*18,[D_gain]*18,[Force]*18,dt,first_one)
                tau=stable_PD_mul.computePD(robot,range(18),goal_pos_25,goal_vel_25,[P_gain]*25,[D_gain]*25,[Force]*25,dt)
                tau1=tau[7:]
                '''
                tau_mul=[]
                for i_index in range(18):
    ...
    with open('/home/xu-ye/Downloads/dynamixel_workbench_master-main(1)/dynamixel_workbench_master-main/dynamixel_workbench_toolbox/test/build/servo_all_fre_cpg_1.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        skip_flag=0
        for row in csv_reader:
    ...
    average_sum=error_sum/all_count
    print("average_sum",average_sum)
    p.disconnect()
