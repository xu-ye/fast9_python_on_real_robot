...
class CPGs_O:
    def __init__(self,theta, dt,b):
        ...
        self.cf_base = 205  # 右边的零位状态　左边增加负号
        self.ft_base = 103
        self.bc_base= 180
        self.old_zz = np.zeros((6,3))
    ...
    def two_layer_out_all(self, z1, t0, tf):
        ...
        return zz_out
    def two_layer_out_all_fre(self, z1, t0, tf,fre):
        self.w_a=fre
        self.w_stance = self.b * self.w_a
        self.w_swing = (1 - self.b) * self.w_a
        z0=np.zeros(12)
    ...
    def cpg2theta_RealRobot(self, zz):
        theta = np.zeros((6, 3))
        for j in range(6):

            if j %2==0 : #左边
                theta[j, 0] = self.bc_base - self.amp_bc * zz[j, 0]
                theta[j, 1] = self.cf_base - self.amp_ft * zz[j, 2]
                theta[j, 2] = self.ft_base + self.amp_ft * zz[j, 4]
            else:# 右边
                theta[j, 0] = self.bc_base + self.amp_bc * zz[j, 0]
        ...
        return np.squeeze(np.resize(theta,(1,18)))
