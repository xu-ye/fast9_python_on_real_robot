...
def __get_observation1(data_read,cpg_index):
    agent_num=6
...
p.resetDebugVisualizerCamera(cameraDistance=0.5, cameraYaw=0,
                             cameraPitch=-70, cameraTargetPosition=[0.2, -0.6, 1])
# 设置每个link的颜色，并且设置初始关节角度
for j in range(numJoints):
    force = 30
    p.setJointMotorControl2(robot, j, p.POSITION_CONTROL, pos[j],force=4)
...
with open('force_real12.json', 'r') as f:
    data_read = json.load(f)
    force_r = np.asarray(data_read['contact_force'])
    phase_r = np.asarray(data_read['phase'])
    cpg_r = np.asarray(data_read['cpg'])
...
for t in t_eval:
    ...
    basePos, baseOri = p.getBasePositionAndOrientation(robot)
    set_position(np.squeeze(theta1))
    # print(phase1)
    # print(t)
    p.stepSimulation()
    count += 1
    ...
    observation0=__get_observation1(data_read,cpg_index)
    end_pos=observation0[:,8:11]
    accu_pos_error=memory(count,end_pos)
    d_error=end_pos-last_error
    last_error=end_pos
    end_pos_n = np.append(end_pos_n, np.resize(end_pos, (1, 6, 3)), axis=0)
    contact_force_diff=observation0[:,7]
    force_diff_n = np.append(force_diff_n, np.resize(contact_force_diff, (1, 6)), axis=0)
    phase_continuous1=observation0[:,14]
    phase_n = np.append(phase_n, np.resize(phase_continuous1, (1, 6)), axis=0)
    ...
    reward_n = np.append(reward_n, np.resize(reward1, (1, 6)), axis=0)
    reward_n_all.append(reward_all)
    start_T = time()
    #print("others:",end_T-start_T)
    '''
    if t>101*dt:
        plt.clf()
        
        plt.subplot(3, 3, 1)
        plt.plot(T[100:], end_pos_n[100:, 0,2]*10000,'-r',T[100:], phase_n[100:, 0],'-b',)
        plt.subplot(3, 3, 2)
        plt.plot(T[100:], end_pos_n[100:, 3,2]*10000,'-r',T[100:], phase_n[100:, 3],'-b',)
        plt.subplot(3, 3, 4)
        plt.plot(T[100:], end_pos_n[100:, 1,2]*10000,'-r',T[100:], phase_n[100:, 1],'-b',)
        plt.subplot(3, 3, 5)
        plt.plot(T[100:], end_pos_n[100:, 4,2]*10000,'-r',T[100:], phase_n[100:, 4],'-b',)
        plt.subplot(3, 3, 7)
        plt.plot(T[100:], end_pos_n[100:, 2,2]*10000,'-r',T[100:], phase_n[100:, 2],'-b',)
        plt.subplot(3, 3, 8)
        plt.plot(T[100:], end_pos_n[100:, 5,2]*10000,'-r',T[100:], phase_n[100:, 5],'-b',)
        plt.subplot(3, 3, 3)
        plt.plot(T[100:], force_diff_n[100:,0],'-r',)
        plt.subplot(3, 3, 6)
        plt.plot(T[100:], force_diff_n[100:,1],'-r',)
        plt.subplot(3, 3, 9)
        plt.plot(T[100:], force_diff_n[100:,2],'-r',)
...
# 写入json文件
'''
with open('force_record4.json', 'w') as f:
    json.dump(data, f, cls=NumpyArrayEncoder)


with open('force_record4.json', 'r') as f:
    data_read = json.load(f)
    force_r = np.asarray(data_read['contact_force'])
    phase_r = np.asarray(data_read['phase'])
    cpg_r = np.asarray(data_read['cpg'])
