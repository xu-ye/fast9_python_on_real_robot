...
if __name__ == '__main__':
    ...
    #send_obs = Process(target=read_servos_send,args=(servos,data_read,cpg_index,q_imu,socket_tcp) )
    #send_obs.start()
   
    #Receive_act = Process(target=receive,args=(socket_tcp,) )
    #Receive_act.start()
    ...
    print("init angle real ",angles_real)
    #servos.write_all_positions_angles(angles_real)
    q_act.put(angles_real)
    
    actuator = Thread(target=actuator_control,args=(q_act,servos) )
    actuator.start()
    ...
    observation,position_Read=read_servos_only(servos,data_read,cpg_index,q_imu,socket_tcp,0)
    ...
    while True:
        ...
        if len(data)>0:
            data_unpack=struct.unpack('<14f',data)
            print("receive:",time.time(),"action: ",data_unpack,"len",len(data))
            actions=np.array(data_unpack).reshape((7,2)) 
            theta_real=cal_angles(actions,cpg_index,data_read,position_Read,q_act)
            #q_act.put(theta_real)
            #servos.write_all_positions_angles(theta_real) 
            if cpg_index<239:
                ...
                cpg_index=0
            observation,position_Read=read_servos_only(servos,data_read,cpg_index,q_imu,socket_tcp,1)
            print("observation",observation)
    ...
    for step in range(240*3):
        ...
        #start_t=time.time()
        observation=read_servos_only(servos,data_read,cpg_index,q_imu,socket_tcp,1)
        #end_t=time.time()
        #print("read_obs time: ",end_t-start_t)
